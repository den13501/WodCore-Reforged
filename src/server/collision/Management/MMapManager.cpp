...
namespace MMAP
{
    ...
    bool MMapManager::loadMapData(uint32 mapId)
    {
        ...
        if (!file)
        {
            TC_LOG_DEBUG("misc", "MMAP:loadMapData: Error: Could not open mmap file '%s'", fileName.c_str());
            return false;
        }
        ...
        if (count != 1)
        {
            TC_LOG_DEBUG("misc", "MMAP:loadMapData: Error: Could not read params from file '%s'", fileName.c_str());
            return false;
        }
        ...
        {
            dtFreeNavMesh(mesh);
            TC_LOG_ERROR("misc", "MMAP:loadMapData: Failed to initialize dtNavMesh for mmap %04u from file %s", mapId, fileName.c_str());
            return false;
        }

        TC_LOG_DEBUG("misc", "MMAP:loadMapData: Loaded %04i.mmap", mapId);
    ...
    bool MMapManager::loadMap(const std::string& /*basePath*/, uint32 mapId, int32 x, int32 y)
    {
        ...
        if (!file)
        {
            TC_LOG_DEBUG("misc", "MMAP:loadMap: Could not open mmtile file '%s'", fileName.c_str());
            return false;
        }
        ...
        {
            TC_LOG_ERROR("misc", "MMAP:loadMap: Bad header in mmap %04u%02i%02i.mmtile", mapId, x, y);
            fclose(file);
            return false;
        }
        ...
        {
            TC_LOG_ERROR("misc", "MMAP:loadMap: %04u%02i%02i.mmtile was built with generator v%i, expected v%i",
        ...
        if (!result)
        {
            TC_LOG_ERROR("misc", "MMAP:loadMap: Bad header or data in mmap %04u%02i%02i.mmtile", mapId, x, y);
            fclose(file);
            return false;
        }
        ...
        if (dtStatusSucceed(mmap->navMesh->addTile(data, fileHeader.size, DT_TILE_FREE_DATA, 0, &tileRef)))
        {
            ...
            ++loadedTiles;
            TC_LOG_DEBUG("misc", "MMAP:loadMap: Loaded mmtile %04i[%02i, %02i] into %04i[%02i, %02i]", mapId, x, y, mapId, header->x, header->y);
        ...
        TC_LOG_ERROR("misc", "MMAP:loadMap: Could not load %04u%02i%02i.mmtile into navmesh", mapId, x, y);
        dtFree(data);
        return false;
    }
    ...
    PhasedTile* MMapManager::LoadTile(uint32 mapId, int32 x, int32 y)
    {
        ...
        if (!file)
        {
            ...
            //TC_LOG_DEBUG("misc", "phase", "MMAP:LoadTile: Could not open mmtile file '%s'", fileName);
            return NULL;
        }
        ...
        {
            TC_LOG_ERROR("misc", "MMAP:LoadTile: Bad header in mmap %04u%02i%02i.mmtile", mapId, x, y);
            fclose(file);
            delete pTile;
            return nullptr;
        }
        ...
        {
            TC_LOG_ERROR("misc", "MMAP:LoadTile: %04u%02i%02i.mmtile was built with generator v%i, expected v%i",
        ...
        if (!result)
        {
            TC_LOG_ERROR("misc", "MMAP:LoadTile: Bad header or data in mmap %04u%02i%02i.mmtile", mapId, x, y);
            fclose(file);
            delete pTile;
            return nullptr;
        }
    ...
    {
        TC_LOG_DEBUG("misc", "MMAP:LoadPhaseTiles: Loading phased mmtiles for map %u, x: %i, y: %i", phasedMapData->first, x, y);
        ...
        for (uint32 phaseMapId : phasedMapData->second)
        {
            ...
            {
                TC_LOG_DEBUG("misc", "MMAP:LoadPhaseTiles: Loaded phased %04u%02i%02i.mmtile for root phase map %u", phaseMapId, x, y, phasedMapData->first);
    ...
    {
        TC_LOG_DEBUG("misc", "MMAP:UnloadPhaseTile: Unloading phased mmtile for map %u, x: %i, y: %i", phasedMapData->first, x, y);
        ...
        for (uint32 phaseMapId : phasedMapData->second)
        {
            ...
            {
                TC_LOG_DEBUG("misc", "MMAP:UnloadPhaseTile: Unloaded phased %04u%02i%02i.mmtile for root phase map %u", phaseMapId, x, y, phasedMapData->first);
    ...
    bool MMapManager::unloadMap(uint32 mapId, int32 x, int32 y)
    {
        ...
        if (itr == loadedMMaps.end())
        {
            ...
            TC_LOG_DEBUG("misc", "MMAP:unloadMap: Asked to unload not loaded navmesh map. %04u%02i%02i.mmtile", mapId, x, y);
            return false;
        }
        ...
        {
            // file may not exist, therefore not loaded
            TC_LOG_DEBUG("misc", "MMAP:unloadMap: Asked to unload not loaded navmesh tile. %04u%02i%02i.mmtile", mapId, x, y);
            return false;
        }
        ...
        if (dtStatusFailed(mmap->navMesh->removeTile(tileRef, NULL, NULL)))
        {
            ...
            TC_LOG_ERROR("misc", "MMAP:unloadMap: Could not unload %04u%02i%02i.mmtile from navmesh", mapId, x, y);
            abort();
        }
        else
        {
            ...
            --loadedTiles;
            TC_LOG_DEBUG("misc", "MMAP:unloadMap: Unloaded mmtile %03i[%02i, %02i] from %04i", mapId, x, y, mapId);
    ...
    bool MMapManager::unloadMap(uint32 mapId)
    {
        ...
        {
            // file may not exist, therefore not loaded
            TC_LOG_DEBUG("misc", "MMAP:unloadMap: Asked to unload not loaded navmesh map %04u", mapId);
            return false;
        }
        ...
        for (MMapTileSet::iterator i = mmap->loadedTileRefs.begin(); i != mmap->loadedTileRefs.end(); ++i)
        {
            ...
            else
            {
                ...
                if (phasedMaps != phaseMapData.end())
                    UnloadPhaseTile(phasedMaps, x, y);
                --loadedTiles;
                TC_LOG_DEBUG("misc", "MMAP:unloadMap: Unloaded mmtile %04i[%02i, %02i] from %04i", mapId, x, y, mapId);
            }
        }

        delete mmap;
        itr->second = nullptr;
        TC_LOG_DEBUG("misc", "MMAP:unloadMap: Unloaded %04i.mmap", mapId);
    ...
    bool MMapManager::unloadMapInstance(uint32 mapId, uint32 instanceId)
    {
        ...
        {
            // file may not exist, therefore not loaded
            TC_LOG_DEBUG("misc", "MMAP:unloadMapInstance: Asked to unload not loaded navmesh map %04u", mapId);
            return false;
        }
        ...
        {
            TC_LOG_DEBUG("misc", "MMAP:unloadMapInstance: Asked to unload not loaded dtNavMeshQuery mapId %04u instanceId %u", mapId, instanceId);
            return false;
        }
        ...
        TC_LOG_DEBUG("misc","MMAP:unloadMapInstance: Unloaded mapId %04u instanceId %u", mapId, instanceId);
    ...
    dtNavMeshQuery const* MMapManager::GetNavMeshQuery(uint32 mapId, uint32 instanceId, TerrainSet swaps)
    {
        ...
        if (mmap->navMeshQueries.find(instanceId) == mmap->navMeshQueries.end())
        {
            ...
            {
                dtFreeNavMeshQuery(query);
                TC_LOG_ERROR("misc", "MMAP:GetNavMeshQuery: Failed to initialize dtNavMeshQuery for mapId %04u instanceId %u", mapId, instanceId);
                return NULL;
            }

            TC_LOG_DEBUG("misc", "MMAP:GetNavMeshQuery: created dtNavMeshQuery for mapId %04u instanceId %u", mapId, instanceId);
    ...
    void MMapData::RemoveSwap(PhasedTile* ptile, uint32 swap, uint32 packedXY)
    {
        ...
        {
            TC_LOG_DEBUG("misc", "MMapData::RemoveSwap: mmtile %04u[%02i, %02i] unload skipped, due to not loaded", swap, x, y);
            return;
        }
        ...
        if (dtStatusFailed(navMesh->removeTile(loadedTileRefs[packedXY], NULL, NULL)))
            TC_LOG_ERROR("misc", "MMapData::RemoveSwap: Could not unload phased %04u%02i%02i.mmtile from navmesh", swap, x, y);
        else
        {
            TC_LOG_DEBUG("misc", "MMapData::RemoveSwap: Unloaded phased %04u%02i%02i.mmtile from navmesh", swap, x, y);

            // restore base tile
            if (dtStatusSucceed(navMesh->addTile(_baseTiles[packedXY]->data, _baseTiles[packedXY]->dataSize, 0, 0, &loadedTileRefs[packedXY])))
            {
                TC_LOG_DEBUG("misc", "MMapData::RemoveSwap: Loaded base mmtile %04u[%02i, %02i] into %04i[%02i, %02i]", _mapId, x, y, _mapId, header->x, header->y);
            }
            else
                TC_LOG_ERROR("misc", "MMapData::RemoveSwap: Could not load base %04u%02i%02i.mmtile to navmesh", _mapId, x, y);
        }
        ...
        if (loadedPhasedTiles[swap].empty())
        {
            ...
            TC_LOG_DEBUG("misc", "MMapData::RemoveSwap: Fully removed swap %u from map %u", swap, _mapId);
        }
    }
    ...
    void MMapData::AddSwap(PhasedTile* ptile, uint32 swap, uint32 packedXY)
    {
        ...
        {
            TC_LOG_DEBUG("misc", "MMapData::AddSwap: phased mmtile %04u[%02i, %02i] load skipped, due to not loaded base tile on map %u", swap, x, y, _mapId);
            return;
        }
        if (loadedPhasedTiles[swap].find(packedXY) != loadedPhasedTiles[swap].end())
        {
            TC_LOG_DEBUG("misc", "MMapData::AddSwap: WARNING! phased mmtile %04u[%02i, %02i] load skipped, due to already loaded on map %u", swap, x, y, _mapId);
            return;
        }
        ...
        if (!oldTile)
        {
            TC_LOG_DEBUG("misc", "MMapData::AddSwap: phased mmtile %04u[%02i, %02i] load skipped, due to not loaded base tile ref on map %u", swap, x, y, _mapId);
            return;
        }
        ...
        {
            TC_LOG_ERROR("misc", "MMapData::AddSwap: Could not unload %04u%02i%02i.mmtile from navmesh", _mapId, x, y);
            delete pt;
        }
        else
        {
            TC_LOG_DEBUG("misc", "MMapData::AddSwap: Unloaded %04u%02i%02i.mmtile from navmesh", _mapId, x, y);
            ...
            {
                TC_LOG_DEBUG("misc","MMapData::AddSwap: Loaded phased mmtile %04u[%02i, %02i] into %04i[%02i, %02i]", swap, x, y, _mapId, header->x, header->y);
            }
            else
                TC_LOG_ERROR("misc", "MMapData::AddSwap: Could not load %04u%02i%02i.mmtile to navmesh", swap, x, y);
        }
    }
