...
void LoadGameObjectModelList()
{
    ...
    if (!model_list_file)
    {
        TC_LOG_ERROR(LOG_FILTER_MAPS, "Unable to open '%s' file.", VMAP::GAMEOBJECT_MODELS);
        return;
    }
    ...
    while (true)
    {
        ...
        {
            TC_LOG_DEBUG(LOG_FILTER_MAPS, "File '%s' seems to be corrupted!", VMAP::GAMEOBJECT_MODELS);
            break;
        }

        if (v1.isNaN() || v2.isNaN())
        {
            TC_LOG_DEBUG(LOG_FILTER_MAPS, "File '%s' Model '%s' has invalid v1%s v2%s values!", VMAP::GAMEOBJECT_MODELS, std::string(buff, name_length).c_str(), v1.toString().c_str(), v2.toString().c_str());
            continue;
        }
    ...
    TC_LOG_DEBUG(LOG_FILTER_MAPS, ">> Loaded %u GameObject models in %u ms", uint32(model_list.size()), GetMSTimeDiffToNow(oldMSTime));
}
...
bool GameObjectModel::initialize(const GameObject& go, const GameObjectDisplayInfoEntry& info)
{
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_MAPS, "GameObject model %s has zero bounds, loading skipped", it->second.name.c_str());
        return false;
    }
