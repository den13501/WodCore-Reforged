...
void IRSocket::CloseSocket (void)
{
    ...
    TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "[INTERREALM] IRSocket::CloseSocket");
}
...
int IRSocket::SendPacket(WorldPacket const* pct)
{
    ...
    else
    {
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_INTERREALM, "IRSocket::SendPacket enqueue_tail failed");
            mb->release();
            return -1;
        }
    }
...
int IRSocket::open (void *a)
{
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "IRSocket::open: peer().get_remote_addr errno = %s", ACE_OS::strerror (errno));
        return -1;
    }
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "IRSocket::open: unable to register client handler errno = %s", ACE_OS::strerror (errno));
        return -1;
    }
...
int IRSocket::handle_input (ACE_HANDLE)
{
    ...
    switch (handle_input_missing_data())
    {
        case -1 :
        {
            ...
            TC_LOG_ERROR(LOG_FILTER_INTERREALM, "IRSocket::handle_input: Peer error closing connection errno = %s", ACE_OS::strerror (errno));
        ...
        case 0:
        {
            TC_LOG_ERROR(LOG_FILTER_INTERREALM, "IRSocket::handle_input: Peer has closed connection");
...
int IRSocket::handle_output_queue (GuardType& g)
{
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "IRSocket::handle_output_queue dequeue_head");
        return -1;
    }
    ...
    else if (n < (ssize_t)send_len) //now n > 0
    {
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_INTERREALM, "IRSocket::handle_output_queue enqueue_head");
            mblk->release();
            return -1;
        }
...
{
    TC_LOG_DEBUG(LOG_FILTER_INTERREALM, "IRSocket::handle_close");
...
int IRSocket::handle_input_header (void)
{
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Header size failed, header size %u.", header.size);
        errno = EINVAL;
        return -1;
    }
...
int IRSocket::handle_input_payload (void)
{
    ...
    if (ret == -1)
    {
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "handle_input_payload failed.");
        errno = EINVAL;
    }
...
int IRSocket::handle_input_missing_data (void)
{
    ...
    while (message_block.length() > 0)
    {
        ...
        if (!m_RecvWPct)
        {
            TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Forcing close on input m_RecvWPct = NULL");
            errno = EINVAL;
            return -1;
        }
...
int IRSocket::cancel_wakeup_output (GuardType& g)
{
        ...
        (this, ACE_Event_Handler::WRITE_MASK) == -1)
    {
        ...
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "IRSocket::cancel_wakeup_output");
        return -1;
    }
...
int IRSocket::schedule_wakeup_output (GuardType& g)
{
    ...
    if (reactor()->schedule_wakeup
        (this, ACE_Event_Handler::WRITE_MASK) == -1)
    {
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "IRSocket::schedule_wakeup_output");
        return -1;
    }
...
int IRSocket::ProcessIncoming(WorldPacket* new_pct)
{
    ...
    {
        /*TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, "[INTERREALM] IRSocket::ProcessIncoming ByteBufferException occured while parsing an instant handled packet %s from client %s. Disconnected client.",
...
int IRSocket::open (void *a)
{
    ...
    if (peer().get_remote_addr(remote_addr) == -1)
    {
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "IRSocket::open: peer().get_remote_addr errno = %s", ACE_OS::strerror (errno));
        return -1;
    }
...
int IRSocket::handle_input (ACE_HANDLE)
{
    ...
    switch (handle_input_missing_data())
    {
        case -1 :
        {
            ...
            TC_LOG_DEBUG(LOG_FILTER_INTERREALM, "IRSocket::handle_input: Peer error closing connection errno = %s", ACE_OS::strerror (errno));
        ...
        case 0:
        {
            TC_LOG_DEBUG(LOG_FILTER_INTERREALM, "IRSocket::handle_input: Peer has closed connection");
...
int IRSocket::ProcessIncoming(WorldPacket* new_pct)
{
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "IRSocket::ProcessIncoming ByteBufferException occured while parsing an instant handled packet %s from client %s. Disconnected client.",
                       opcodeName.c_str(), GetRemoteAddress().c_str());
        new_pct->hexlike();
        return -1;
    }
