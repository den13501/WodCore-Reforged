    ...
    try
    {
        switch (opcode)
        {
            ...
            case CMSG_AUTH_SESSION:
            {
                if (m_Session)
                {
                    TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "WorldSocket::ProcessIncoming: received duplicate CMSG_AUTH_SESSION from %s", m_Session->GetPlayerName(false).c_str());
                    return -1;
                }
            ...
            {
                TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "%s", GetOpcodeNameForLogging(opcode, WOW_CLIENT_TO_SERVER).c_str());
                sScriptMgr->OnPacketReceive(this, WorldPacket(*new_pct));
                return 0;
            }
...
int WorldSocket::HandleAuthSession(WorldPacket& p_RecvPacket)
{
    ...
    TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthSession: (s,v) check s: %s v: %s", l_Fields[5].GetCString(), l_Fields[4].GetCString());
    ...
    TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "Allowed Level: %u Player Level %u", l_AllowedAccountType, AccountTypes(l_AccountGMLevel));
    ...
    {
        SendAuthResponse(AUTH_FAILED, false, 0);
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthSession: Authentication failed for account: %u ('%s') address: %s", l_AccountID, l_AccountName.c_str(), l_SessionIP.c_str());

        return -1;
    }

    TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthSession: Client '%s' authenticated successfully from %s.", l_AccountName.c_str(), l_SessionIP.c_str());
