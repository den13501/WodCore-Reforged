...
# define CheckSocket m_Socket
        ...
        TC_LOG_INFO("entities.player.character", "Account: %d (IP: %s) Logout Character:[%s] (GUID: %u) Level: %d", GetAccountId(), GetRemoteAddress().c_str(), m_Player->GetName(), m_Player->GetGUIDLow(), m_Player->getLevel());
...
void WorldSession::LoadCharacterDone(LoginQueryHolder* p_CharHolder, LoginDBQueryHolder* p_AuthHolder)
{
    ...
    if (!GetPlayer()->LoadFromDB(GUID_LOPART(l_PlayerGuid), p_CharHolder, p_AuthHolder))
    {
        ...
        TC_LOG_INFO("interrealm", "Cannot load player in WorldSession::LoadCharacterDone.");
