        ...
        try
        {
            switch (opHandle->status)
            {
                case STATUS_LOGGEDIN:
                    ...
                    {
                        //TC_LOG_ERROR(LOG_FILTER_SERVER_LOADING, "Packet received when in IRBG: %x (%s)", packet->GetOpcode(), opcodeTable[packet->GetOpcode()]->Name);
...
void WorldSession::SendAddonsInfo()
{
    ...
    for (AddonsList::iterator l_It = m_addonsList.begin(); l_It != m_addonsList.end(); ++l_It)
    {
        ...
        if (l_UsePublicKey)
        {
            TC_LOG_INFO(LOG_FILTER_GENERAL, "ADDON: CRC (0x%x) for addon %s is wrong (does not match expected 0x%x), sending pubkey", l_It->CRC, l_It->Name.c_str(), STANDARD_ADDON_CRC);
...
void WorldSession::LoadCharacterDone(LoginQueryHolder* p_CharHolder, LoginDBQueryHolder* p_AuthHolder)
{
    ...
    if (!GetPlayer()->LoadFromDB(GUID_LOPART(l_PlayerGuid), p_CharHolder, p_AuthHolder))
    {
        ...
        TC_LOG_INFO(LOG_FILTER_INTERREALM, "Cannot load player in WorldSession::LoadCharacterDone.");
