...
int InterRealmSession::OnSocketOpen(IRSocket* socket)
{
    ...
    if (m_SockOutKBuff >= 0)
    {
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_INTERREALM, "InterRealmSession::OnSocketOpen set_option SO_SNDBUF");
            return -1;
        }
    }
    ...
    if (m_UseNoDelay)
    {
        if (socket->peer().set_option (ACE_IPPROTO_TCP,
            ...
            sizeof (int)) == -1)
        {
            TC_LOG_ERROR(LOG_FILTER_INTERREALM, "InterRealmSession::OnSocketOpen peer().set_option TCP_NODELAY errno = %s", ACE_OS::strerror (errno));
            return -1;
        }
    }
...
{
    TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Connecting to InterRealm...");

    if (!sWorld->getBoolConfig(CONFIG_INTERREALM_ENABLE))
    {
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "InterRealm is disabled.");
        return;
    }
    ...
    TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Loaded InterRealm configuration, %s:%u, id %u", m_IP.c_str(), m_port, m_ir_id);    
    ...
    while (!World::IsStopped())
    {
        if (!m_IRSocket || m_IRSocket->IsClosed())
        {
            ...
            TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Trying to connect to interrealm.");
            ...
            {
                ClearSocket();
                TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Cannot connect interrealm");    
            ...
            TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Sending 'hello' message to InterRealm.");
...
void InterRealmSession::Handle_Hello(WorldPacket& packet)
{
    ...
    {
       TC_LOG_ERROR(LOG_FILTER_INTERREALM, "closing socket !");
        m_force_stop = true;
    }

    if (_rand != m_rand)
    {
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Random hello check is incorrect, closing socket");
        m_force_stop = true;
    }

    if (_resp == IR_HELO_RESP_PROTOCOL_MISMATCH)
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "InterRealm Protocol Mismatch, closing doors to me !");

    if(_resp == IR_HELO_RESP_POLITE)
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Server like to be polite, closing doors to me !");
...
void InterRealmSession::Handle_WhoAmI(WorldPacket& packet)
{
    ...
    if (_valid == 0)
    {
        ...
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Tunnel is opened.");
    ...
    {
        m_force_stop = true;
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Registration was failed.");
    }
}
...
{
    /*TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Received IR_SMSG_DISTRIBUTE_ARENA_POINTS_RESP.");
...
{
    TC_LOG_INFO(LOG_FILTER_INTERREALM, "Received guild query for new guilds.");
    ...
    while (m_IRSocket && !m_IRSocket->IsClosed() && !_queue.empty() && _queue.next(packet))
    {
        ...
        if (packet->GetOpcode() < IR_NUM_MSG_TYPES)
        {
            try
            {
                ...
                if (!IRopHandle)
                {
                    TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Cannot find handle for the opcode (%u). Skipped packet.",
                    packet->GetOpcode());
                    continue;
                }
            ...
            {
                TC_LOG_ERROR(LOG_FILTER_INTERREALM, "InterRealmSession ByteBufferException occured while parsing a packet (opcode: %u). Skipped packet.",
                    packet->GetOpcode());
            }
