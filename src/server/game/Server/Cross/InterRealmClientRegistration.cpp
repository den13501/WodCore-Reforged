...
void InterRealmClient::Handle_RegisterPlayer(WorldPacket& p_Packet)
{
    ...
    if (!l_HasExtistedPlayer)
    {
        ...
        if (!l_Player)
        {
            TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Failed to create a player.");
            p_Packet.rfinish();
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "Battleground: invalid bgtype (%u) received. possible cheater? player guid %u", l_BGType, l_Player->GetGUIDLow());
        return;
    }
...
{
    TC_LOG_DEBUG(LOG_FILTER_INTERREALM, "Received packet IR_CMSG_REGISTER_GROUP");
    ...
    TC_LOG_DEBUG(LOG_FILTER_INTERREALM, "Succeed to register a group.");
}

void InterRealmClient::Handle_RegisterArena(WorldPacket& p_Packet)
{
    TC_LOG_DEBUG(LOG_FILTER_INTERREALM, "Received packet IR_CMSG_REGISTER_ARENA");
...
{
    TC_LOG_DEBUG(LOG_FILTER_INTERREALM, "Received packet IR_CMSG_REGISTER_RATED");
    ...
    if (!l_Battleground || DisableMgr::IsDisabledFor(DISABLE_TYPE_BATTLEGROUND, BATTLEGROUND_RATED_10_VS_10, NULL))
    {
        ...
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Battleground: template bg (all arenas) not found or disable");
        return;
    }
...
void InterRealmClient::Handle_AppearRequest(WorldPacket& packet)
{
    ...
    if (playerGuid == 0)
    {
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Wrong player guid (%u), realm %u %s",
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Failed to appear becouse player is already at the cross-server.");
        packet.rfinish();
        return;
    }
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Failed to appear becouse targeted player is not at the cross-server.");
        packet.rfinish();
        return;
    }
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Failed to appear becouse targeted player is not at the arena.");
        packet.rfinish();
        return;
    }
    ...
    if (!player)
    {
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Failed to appear.");
        packet.rfinish();
        return;
    }
...
void InterRealmClient::Handle_BattlefieldPort(WorldPacket& packet)
{
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "BattlegroundHandler: itrplayerstatus not found.");
        return;
    }
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "BattlegroundHandler: instance not found.");
        return;
    }
    ...
    if (!l_BG)
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "BattlegroundHandler: bg_template not found for type id %u.", l_BGQueueTypeID);
        return;
    }
    ...
    if (l_Action == 1 && l_GroupQueueInfo.m_ArenaType == 0)
    {
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "Battleground: Player %s (%u) has level (%u) higher than maxlevel (%u) of battleground (%u)! Do not port him to battleground!",
    ...
    switch (l_Action)
    {
        ...
        case 0:
            ...
            break;
        default:
            TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "Battleground port: unknown action %u", l_Action);
            break;
    }
}
...
{
    TC_LOG_DEBUG(LOG_FILTER_INTERREALM, "Received packet IR_CMSG_BATTLEFIELD_ENTER");
