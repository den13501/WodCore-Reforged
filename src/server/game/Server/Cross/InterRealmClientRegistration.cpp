...
void InterRealmClient::Handle_RegisterPlayer(WorldPacket& p_Packet)
{
    ...
    if (!l_HasExtistedPlayer)
    {
        ...
        if (!l_Player)
        {
            TC_LOG_ERROR("interrealm", "Failed to create a player.");
            p_Packet.rfinish();
...
{
    TC_LOG_DEBUG("interrealm", "Received packet IR_CMSG_REGISTER_GROUP");
    ...
    TC_LOG_DEBUG("interrealm", "Succeed to register a group.");
}

void InterRealmClient::Handle_RegisterArena(WorldPacket& p_Packet)
{
    TC_LOG_DEBUG("interrealm", "Received packet IR_CMSG_REGISTER_ARENA");
...
{
    TC_LOG_DEBUG("interrealm", "Received packet IR_CMSG_REGISTER_RATED");
    ...
    if (!l_Battleground || DisableMgr::IsDisabledFor(DISABLE_TYPE_BATTLEGROUND, BATTLEGROUND_RATED_10_VS_10, NULL))
    {
        ...
        TC_LOG_ERROR("interrealm", "Battleground: template bg (all arenas) not found or disable");
        return;
    }
...
void InterRealmClient::Handle_AppearRequest(WorldPacket& packet)
{
    ...
    if (playerGuid == 0)
    {
        TC_LOG_ERROR("interrealm", "Wrong player guid (%u), realm %u %s",
    ...
    {
        TC_LOG_ERROR("interrealm", "Failed to appear becouse player is already at the cross-server.");
        packet.rfinish();
        return;
    }
    ...
    {
        TC_LOG_ERROR("interrealm", "Failed to appear becouse targeted player is not at the cross-server.");
        packet.rfinish();
        return;
    }
    ...
    {
        TC_LOG_ERROR("interrealm", "Failed to appear becouse targeted player is not at the arena.");
        packet.rfinish();
        return;
    }
    ...
    if (!player)
    {
        TC_LOG_ERROR("interrealm", "Failed to appear.");
        packet.rfinish();
        return;
    }
...
{
    TC_LOG_DEBUG("interrealm", "Received packet IR_CMSG_BATTLEFIELD_ENTER");
