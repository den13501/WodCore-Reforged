...
void InterRealmClient::SendPacket(WorldPacket const* packet)
{
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Cannot send packet %u", packet->GetOpcode());
        m_IRSocket->CloseSocket();
    }
}
...
{
    TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Unhandled Packet with IROpcode %u received !", recvPacket.GetOpcode());
}

void InterRealmClient::Handle_Null(WorldPacket& recvPacket)
{
    recvPacket.rfinish();
    //TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Packet with Invalid IROpcode %u received !", recvPacket.GetOpcode());
}

void InterRealmClient::Handle_Hello(WorldPacket& packet)
{
    TC_LOG_DEBUG(LOG_FILTER_INTERREALM, "Received packet CMSG_HELLO.", m_realmId);
    ...
    {
        m_isNeedClose = true;
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Non-polite server %s, rejecting.", hello.c_str());
    }
    ...
    TC_LOG_DEBUG(LOG_FILTER_INTERREALM, "Send packet SMSG_HELLO.", m_realmId);
...
void InterRealmClient::Handle_TunneledPacket(WorldPacket* recvPacket)
{
    ...
    *recvPacket >> opcodeId;
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "no player");
        delete recvPacket;
        recvPacket = NULL;
        return;
    }
...
void InterRealmClient::Handle_WhoAmI(WorldPacket& packet)
{
    ...
    TC_LOG_DEBUG(LOG_FILTER_INTERREALM, "Received packet CMSG_WHO_AM_I.", m_realmId);
    ...
    if (!config)
    {
        TC_LOG_INFO(LOG_FILTER_INTERREALM, "InterRealmClient %u has no config.", m_realmId);
        pckt << uint8(1);
        SendPacket(&pckt);
        m_isNeedClose = true;
        return;
    ...
    {
        TC_LOG_INFO(LOG_FILTER_INTERREALM, "Database connection was failed for InterRealmClient %u.", m_realmId);
        pckt << uint8(1);
        SendPacket(&pckt);
        m_isNeedClose = true;
        return;
    ...
    TC_LOG_INFO(LOG_FILTER_INTERREALM, "Database connection was successful. %lu", time(nullptr));
...
{
    TC_LOG_DEBUG(LOG_FILTER_INTERREALM, "Received packet IR_CMSG_CHECK_PLAYERS");
    ...
    if (num_players > 0)
    {
        for (uint16 i = 0; i < num_players; ++i)
        {
            uint64 playerGuid;
            packet >> playerGuid;
            TC_LOG_DEBUG(LOG_FILTER_INTERREALM, "Connection lost with player %u", playerGuid);
...
void InterRealmClient::Handle_PlayerLogout(WorldPacket& packet)
{
    ...
    TC_LOG_DEBUG(LOG_FILTER_INTERREALM, "Logout player %u", playerGuid);
...
{
    TC_LOG_DEBUG(LOG_FILTER_INTERREALM, "Received packet IR_CMSG_BATTLEGROUND_LIST_QUERY");
...
{
    TC_LOG_DEBUG(LOG_FILTER_INTERREALM, "Received packet IR_CMSG_BATTLEFIELD_STATUS_QUERY");
...
{
    TC_LOG_DEBUG(LOG_FILTER_INTERREALM, "Received packet IR_CMSG_CURRENCY_CAP_LOAD");
...
{
    TC_LOG_DEBUG(LOG_FILTER_INTERREALM, "Received packet IR_CMSG_CURRENCY_CAP_RESET");
...
{
    TC_LOG_DEBUG(LOG_FILTER_INTERREALM, "Received packet IR_MSG_WHISPER");
...
void InterRealmClient::Handle_TunneledClient(WorldPacket &recvPacket)
{
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Tunneled packet was sended for unregistered player.");
        return;
    }
...
void InterRealmClient::Handle_TeleportPlayer(WorldPacket& recvPacket)
{
    ...
    if (itr == m_Players.end())
    {
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Failed to register a player.");
    ...
    else
    {
        ...
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Failed to teleport a player.");
        return;
    }
    ...
    if (!player->LoadFromDB(playerGUIDLow))
    {
        ...
        TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Could not load a player %u", playerGUIDLow);
...
{
    TC_LOG_DEBUG(LOG_FILTER_INTERREALM, "Received packet IR_CMSG_RATED_BATTLEGROUND_STATS");
    ...
    while (m_IRSocket && !m_IRSocket->IsClosed() && !_queue.empty() && _queue.next(packet))
    {
        if (packet->GetOpcode() < IR_NUM_MSG_TYPES)
        {
            try
            {
                ...
                if (!opHandle)
                {
                    TC_LOG_INFO(LOG_FILTER_INTERREALM, "Cannot find handle for the opcode (%u). Skipped packet.",
                    packet->GetOpcode());
                    continue;
                }
            ...
            {
                TC_LOG_ERROR(LOG_FILTER_INTERREALM, "InterRealmClient ByteBufferException occured while parsing a packet (opcode: %u) from InterRealm Server. Skipped packet.",
                    packet->GetOpcode());
            }    
            if (packet->rpos() < packet->wpos())
                TC_LOG_ERROR(LOG_FILTER_INTERREALM, "Unprocessed tail data (read stop at %u from %u) in opcode %s", packet->rpos(), packet->wpos(), IRopcodeTable[packet->GetOpcode()]->name);
        }
        else
