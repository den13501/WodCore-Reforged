...
void ObjectMgr::LoadCreatureTemplatesDifficulties()
{
    ...
    if (!l_Result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 creature template difficulties definitions. DB table `creature_template_difficulty` is empty.");
        return;
    }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u creature difficulties definitions in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadCreatureTemplateAddons()
{
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u creature template addons in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::CheckCreatureTemplate(CreatureTemplate const* cInfo)
{
    ...
    for (uint32 diff = 0; diff < Difficulty::MaxDifficulties - 1 && ok; ++diff)
    {
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "Creature (Entry: %u) has different `npcflag2` in difficulty %u mode (Entry: %u).", cInfo->Entry, diff + 1, cInfo->DifficultyEntry[diff]);
            continue;
        }
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_SQL, "Table `creature_template` lists creature (Entry: %u) with `exp_req` %u. Ignored and set to 0.", cInfo->Entry, cInfo->RequiredExpansion);
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_SQL, "Creature(Entry: %u) has a non - existing vignette id (%u)", cInfo->Entry, cInfo->VignetteID);
...
void ObjectMgr::LoadCreatureAddons()
{
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u creature addons in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadEquipmentTemplates()
{
    ...
    do
    {
        ...
        if (!id)
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "Creature equipment template with ID 0 found for creature %u, skipped.", entry);
            continue;
        }
...
bool ObjectMgr::AddGOData(uint32 p_LowGuid, uint32 entry, uint32 mapId, float x, float y, float z, float o, uint32 spawntimedelay, float rotation0, float rotation1, float rotation2, float rotation3)
{
    ...
    TC_LOG_DEBUG(LOG_FILTER_MAPS, "AddGOData: dbguid %u entry %u map %u x %f y %f z %f o %f", p_LowGuid, entry, mapId, x, y, z, o);
...
void ObjectMgr::LoadRealmCompletedChallenges()
{
    ...
    if (!l_Result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 group completed challenges. DB table `group_completed_challenges` is empty.");
        return;
    }
    ...
    if (!l_Result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 guild completed challenges. DB table `guild_completed_challenges` is empty.");
        return;
    }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u realm completed challenges in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadChallengeRewards()
{
    ...
    if (!l_Result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 challenge rewards. DB table `challenge_mode_rewards` is empty.");
        return;
    }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u challenge mode rewards in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadItemTemplates()
{
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u item templates from Item-sparse.db2 in %u ms", sparseCount, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadItemSpecs()
{
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u item specs in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadItemSpecsOverride()
{
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u item specs override in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadAreaTriggerMoveTemplates()
{
    ...
    if (!l_Result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 Areatrigger Move Template in %u ms", GetMSTimeDiffToNow(l_OldMSTime));
        return;
    }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u Areatrigger Move Template in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadAreaTriggerMoveSplines()
{
    ...
    if (!l_Result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 Areatrigger Move Splines in %u ms", GetMSTimeDiffToNow(l_OldMSTime));
        return;
    }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u Areatrigger Move Splines in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadAreaTriggerTemplates()
{
    ...
    if (!l_Result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 Areatrigger template in %u ms", GetMSTimeDiffToNow(l_OldMSTime));
        return;
    }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u Areatrigger templates in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadPetStatInfo()
{
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u pet stats definitions in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadPlayerInfo()
{
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, "Loading Player Create Cast Spell Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
            TC_LOG_ERROR(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 player create cast spells. DB table `playercreateinfo_cast_spell` is empty.");
        else
        {
            uint32 count = 0;
            ...
            do
            {
                ...
                {
                    TC_LOG_ERROR(LOG_FILTER_SQL, "Wrong race mask %u in `playercreateinfo_cast_spell` table, ignoring.", raceMask);
                    continue;
                }

                if (classMask != 0 && !(classMask & CLASSMASK_ALL_PLAYABLE))
                {
                    TC_LOG_ERROR(LOG_FILTER_SQL, "Wrong class mask %u in `playercreateinfo_cast_spell` table, ignoring.", classMask);
                    continue;
                }
            ...
            TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u player create cast spells in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, "Loading Player Create XP Data...");
    {
        ...
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u xp for level definitions in %u ms", l_Count, GetMSTimeDiffToNow(oldMSTime));
    }
}
...
void ObjectMgr::LoadQuests()
{
    ...
    for (QuestMap::iterator iter = _questTemplates.begin(); iter != _questTemplates.end(); ++iter)
    {
        ...
        if (qinfo->GetQuestMethod() >= 4)
            TC_LOG_ERROR(LOG_FILTER_SQL, "Quest %u has `Method` = %u, expected values are 0, 1, 2 or 3.", qinfo->GetQuestId(), qinfo->GetQuestMethod());

        if (qinfo->IsAutoComplete() && qinfo->IsRepeatable())
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "Quest %u is auto-complete and is repeatable", qinfo->GetQuestId());
        ...
        if (qinfo->Flags & QUEST_FLAGS_AUTO_REWARDED)
        {
            ...
            for (int j = 1; j < QUEST_REWARD_CHOICES_COUNT; ++j)
            {
                ...
                {
                    TC_LOG_ERROR(LOG_FILTER_SQL, "Quest %u has `RewardChoiceItemId%d` = %u but item from `RewardChoiceItemId%d` can't be rewarded with quest flag QUEST_FLAGS_AUTO_REWARDED.",
        ...
        for (uint8 j = 0; j < QUEST_REWARDS_COUNT; ++j)
        {
            ...
            if (id)
            {
                ...
                {
                    TC_LOG_ERROR(LOG_FILTER_SQL, "Quest %u has `RewardItemId%d` = %u but `RewardItemCount%d` = 0, quest will not reward this item.",
                        qinfo->GetQuestId(), j+1, id, j+1);
                    // No changes
                }
            }
            ...
            {
                TC_LOG_ERROR(LOG_FILTER_SQL, "Quest %u has `RewardItemId%d` = 0 but `RewardItemCount%d` = %u.",
...
void ObjectMgr::LoadInstanceEncounters()
{
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "Table `instance_encounters` has an invalid encounter id %u, skipped!", l_EncounterID);
            continue;
        }

        if (l_LastEncounterDungeon && !sLFGDungeonStore.LookupEntry(l_LastEncounterDungeon))
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "Table `instance_encounters` has an encounter %u (%s) marked as final for invalid dungeon id %u, skipped!", l_EncounterID, l_DungeonEncounter->NameLang, l_LastEncounterDungeon);
            continue;
        }
        ...
        if (l_LastEncounterDungeon)
        {
            ...
            {
                TC_LOG_ERROR(LOG_FILTER_SQL, "Table `instance_encounters` specified encounter %u (%s) as last encounter but %u (%s) is already marked as one, skipped!", l_EncounterID, l_DungeonEncounter->NameLang, l_Itr->second->ID, l_Itr->second->NameLang);
                continue;
            }
        ...
        switch (l_CreditType)
        {
            case ENCOUNTER_CREDIT_KILL_CREATURE:
            {
                ...
                if (!l_CreatureInfo)
                {
                    TC_LOG_ERROR(LOG_FILTER_SQL, "Table `instance_encounters` has an invalid creature (entry %u) linked to the encounter %u (%s), skipped!", l_CreditEntry, l_EncounterID, l_DungeonEncounter->NameLang);
                    continue;
                }
            ...
            case ENCOUNTER_CREDIT_CAST_SPELL:
                ...
                {
                    TC_LOG_ERROR(LOG_FILTER_SQL, "Table `instance_encounters` has an invalid spell (entry %u) linked to the encounter %u (%s), skipped!", l_CreditEntry, l_EncounterID, l_DungeonEncounter->NameLang);
                    continue;
                }
                break;
            default:
                TC_LOG_ERROR(LOG_FILTER_SQL, "Table `instance_encounters` has an invalid credit type (%u) for encounter %u (%s), skipped!", l_CreditType, l_EncounterID, l_DungeonEncounter->NameLang);
                continue;
        }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u instance encounters in %u ms", l_Counter, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadAccessRequirements()
{
    ...
    do
    {
        ...
        if (ar.leader_achievement)
        {
            ...
            {
                TC_LOG_ERROR(LOG_FILTER_SQL, "Required Leader achievement %u not exist for map %u difficulty %u, remove quest done requirement.", ar.leader_achievement, mapid, difficulty);
...
void ObjectMgr::LoadLFRAccessRequirements()
{
    ...
    if (!l_Result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 LFR access requirement definitions. DB table `lfr_access_requirement` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (l_LFRAccessReq.Item)
        {
            ...
            if (!l_ItemTemplate)
            {
                TC_LOG_ERROR(LOG_FILTER_GENERAL, "Key item %u does not exist for dungeon ID %u, removing key requirement.", l_LFRAccessReq.Item, l_DungeonID);
        ...
        if (l_LFRAccessReq.Item2)
        {
            ...
            if (!l_ItemTemplate)
            {
                TC_LOG_ERROR(LOG_FILTER_GENERAL, "Second item %u does not exist for dungeon ID %u, removing key requirement.", l_LFRAccessReq.Item2, l_DungeonID);
        ...
        if (l_LFRAccessReq.QuestA)
        {
            ...
            {
                TC_LOG_ERROR(LOG_FILTER_SQL, "Required Alliance Quest %u not exist for dungeon ID %u, remove quest done requirement.", l_LFRAccessReq.QuestA, l_DungeonID);
        ...
        if (l_LFRAccessReq.QuestH)
        {
            ...
            {
                TC_LOG_ERROR(LOG_FILTER_SQL, "Required Horde Quest %u not exist for dungeon ID %u, remove quest done requirement.", l_LFRAccessReq.QuestH, l_DungeonID);
        ...
        if (l_LFRAccessReq.Achievement)
        {
            ...
            {
                TC_LOG_ERROR(LOG_FILTER_SQL, "Required Achievement %u not exist for dungeon ID %u, remove quest done requirement.", l_LFRAccessReq.Achievement, l_DungeonID);
        ...
        if (l_LFRAccessReq.LeaderAchievement)
        {
            ...
            {
                TC_LOG_ERROR(LOG_FILTER_SQL, "Required Leader achievement %u not exist for dungeon ID %u, remove quest done requirement.", l_LFRAccessReq.LeaderAchievement, l_DungeonID);
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u lfr access requirement definitions in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadGarrisonPlotBuildingContent()
{
    ...
    if (!l_Result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 garrison plot building content. DB table `garrison_plot_building_content` is empty.");
        return;
    }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u garrison plot building content in %u ms", l_Count, GetMSTimeDiffToNow(l_StartTime));
}
...
void ObjectMgr::LoadNpcRecipesConditions()
{
    ...
    if (!l_Result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 npc recipe condition. DB table `npc_recipe_condition` is empty.");
        return;
    }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u npc recipe condition in %u ms", l_Count, GetMSTimeDiffToNow(l_StartTime));
}
...
#ifndef CROSS
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded the max pet number: %d in %u ms", m_PetNumber-1, GetMSTimeDiffToNow(oldMSTime));
#endif /* not CROSS */
...
void ObjectMgr::LoadCurrencyOnKill()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 creature currency definitions. DB table `creature_currency` is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "Table `creature_creature` have data for not existed creature entry (%u), skipped", l_Creature_id);
            continue;
        }
        ...
        for (CurrencyOnKillEntry::const_iterator i = l_CurrOnKill.begin(); i != l_CurrOnKill.end(); ++i)
        {
            ...
            {
                TC_LOG_ERROR(LOG_FILTER_SQL, "CurrencyType (CurrencyTypes.dbc) %u does not exist but is used in `creature_currency`", i->first);
                continue;
            }
        }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u creature currency definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadPersonnalCurrencyOnKill()
{
    ...
    if (!l_Result)
    {
        TC_LOG_ERROR(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 personnal creature currency definitions. DB table `creature_loot_currency_personnal` is empty.");
        return;
    }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u personnal creature currency definitions in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadNPCSpellClickSpells()
{
    ...
    for (uint32 l_Entry = 0; l_Entry < l_LastEntry; l_Entry++)
    {
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "npc_spellclick_spells: Creature template %u has UNIT_NPC_FLAG_SPELLCLICK but no data in spellclick table! Removing flag", l_CreatureTemplate->Entry);
...
void ObjectMgr::LoadCreatureQuestStarters()
{
    ...
    for (QuestRelations::const_iterator l_Iterator = _creatureQuestRelations.begin(); l_Iterator != _creatureQuestRelations.end(); ++l_Iterator)
    {
        ...
        if (!cInfo)
            TC_LOG_ERROR(LOG_FILTER_SQL, "Table `creature_queststarter` have data for not existed creature entry (%u) and existed quest %u", l_Iterator->first, l_Iterator->second);
        else if (!(cInfo->NpcFlags1 & UNIT_NPC_FLAG_QUESTGIVER))
            TC_LOG_ERROR(LOG_FILTER_SQL, "Table `creature_queststarter` has creature entry (%u) for quest %u, but npcflag does not include UNIT_NPC_FLAG_QUESTGIVER", l_Iterator->first, l_Iterator->second);
...
bool ObjectMgr::IsVendorItemValid(uint32 vendor_entry, uint32 id, int32 maxcount, uint32 incrtime, uint32 ExtendedCost, uint8 type, Player* player, std::set<uint32>* skip_vendors, uint32 ORnpcflag) const
{
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_SQL, "Table `(game_event_)npc_vendor` have Item (Entry: %u, type: %u) with missing maxcount for vendor (%u), ignore", id, type, ExtendedCost, vendor_entry); ///<  Data argument not used by format string
        return false;
    }
    ...
    for (uint32 l_Entry = 0; l_Entry < l_LastEntry; l_Entry++)
    {
        ...
        for (uint16 lvl = l_CreatureTemplate->minlevel; lvl <= l_CreatureTemplate->maxlevel; ++lvl)
        {
            if (_creatureBaseStatsStore.find(MAKE_PAIR16(lvl, l_CreatureTemplate->unit_class)) == _creatureBaseStatsStore.end())
                TC_LOG_ERROR(LOG_FILTER_SQL, "Missing base stats for creature class %u level %u", l_CreatureTemplate->unit_class, lvl);
        }
    }
...
void ObjectMgr::LoadCreatureGroupSizeStats()
{
    ...
    if (!l_Result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 creature group size stats. DB table `creature_groupsizestats` is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "Creature template entry (entry: %u) used in `creature_groupsizestats` does not exist.", l_CreatureEntry);
            continue;
        }

        if (l_Difficulty >= MaxDifficulties)
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "Difficulty %u (entry %u) used in `creature_groupsizestats` is invalid.", l_Difficulty, l_CreatureEntry);
            continue;
        }

        if (l_GroupSize >= MAX_GROUP_SCALING)
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "Group size %u (entry %u) used in `creature_groupsizestats` is invalid.", l_GroupSize, l_CreatureEntry);
            continue;
        }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u creature group size stats in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadFactionChangeQuests()
{
    ...
    if (!l_Result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 faction change quest pairs. DB table `player_factionchange_quests` is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "Alliance quest %u referenced in `player_factionchange_quests` does not exist, pair skipped!", l_AllianceQuest);
            continue;
        }

        if (!sObjectMgr->GetQuestTemplate(l_HordeQuest))
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "Horde quest %u referenced in `player_factionchange_quests` does not exist, pair skipped!", l_HordeQuest);
            continue;
        }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u faction change quest pairs in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadHotfixData(bool p_Reload)
{
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u hotfix info entries in %u ms", l_Count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadBattlePetTemplate()
{
    ...
    if (!result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 battlepet template. DB table `battlepet_template` is empty.");
        return;
    }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u battlepet template in %u ms.", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadBattlePetNpcTeamMember()
{
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u battlepet npc team member in %u ms.", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::ComputeBattlePetSpawns()
{
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> ComputeBattlePetSpawns %u ms.", GetMSTimeDiffToNow(l_OldMSTime));
 ...
 void ObjectMgr::LoadResearchSiteZones()
{
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %lu Archeology research site zones in %u ms.", (unsigned long)sResearchSiteSet.size(), GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadResearchSiteLoot()
{
    ...
    if (!result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 Archeology site loot. DB table `research_loot` is empty.");
        return;
    }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %lu Archeology research site loot in %u ms.", (unsigned long)_researchLoot.size(), GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadSkipUpdateZone()
{
    ...
    while (result->NextRow());
     ...
     TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u zone skip update.", count);
}
...
void ObjectMgr::RestructCreatureGUID(uint32 nbLigneToRestruct)
{
    ...
    if (!result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, "Soucis lors du select de la fonction 'restructCreatureGUID' (nombre de lignes : %u)", nbLigneToRestruct);
        return;
    }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, "%u lignes ont ete restructuree.", nbLigneToRestruct);
}
...
void ObjectMgr::RestructGameObjectGUID(uint32 nbLigneToRestruct)
{
    ...
    if (!result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, "Soucis lors du select de la fonction 'RestructGameObjectGUID' (nombre de lignes : %u)", nbLigneToRestruct);
        return;
    }
...
void ObjectMgr::LoadGuildChallengeRewardInfo()
{
    ...
    if (!result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 guild challenge reward data.");
        return;
    }
    ...
    do
    {
        ...
        {
            TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> guild_challenge_reward has unknown challenge type %u, skip.", type);
            continue;
        }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u guild challenge reward data in %u ms.", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadCharacterTemplateData()
{
    ...
    if (!l_Result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 character templates.");
        return;
    }
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "Template %u defined in `character_template` with class %u does not exists, skipped.", l_ID, l_CharacterTemplate->m_PlayerClass);
        ...
        if (!l_ID)
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "Template %u defined in `character_template` cannot have null index, skipped.", l_ID);
            delete l_CharacterTemplate;
            continue;
        }
        ...
        if (l_ItemResult)
        {
            do
            {
                ...
                if (!l_ItemTemplate)
                {
                    TC_LOG_ERROR(LOG_FILTER_SQL, "ItemID %u defined in `character_template_item` does not exist, ignoring.", l_TemplateItem.m_ItemID);
                    continue;
                }
        ...
        if (l_SpellsResult)
        {
            do
            {
                ...
                {
                    TC_LOG_ERROR(LOG_FILTER_SQL, "SpellId %u defined in `character_template_spell` does not exist, ignoring.", l_SpellID);
                    continue;
                }
        ...
        if (l_ReputationResult)
        {
            do
            {
                ...
                {
                    TC_LOG_ERROR(LOG_FILTER_SQL, "FactionID %u defined in `character_template_reputation` does not exist, ignoring.", l_FactionID);
                    continue;
                }
    ...
    while (l_Result->NextRow());
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u character templates %u ms.", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadQuestObjectives()
{
    ...
    if (!l_Result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 Quest Objectives. DB table `quest_objective` is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "Quest Objective %u has non existant Quest Id %u! Skipping.", l_ObjectiveID, l_ObjectiveQuestId);
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "Quest Objective %u has invalid type %u! Skipping.", l_ObjectiveID, l_ObjectiveType);
            m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
            continue;
        }
        ...
        switch (l_ObjectiveType)
        {
            ...
            case QUEST_OBJECTIVE_TYPE_PET_BATTLE_TAMER:
            {
                ...
                {
                    TC_LOG_ERROR(LOG_FILTER_SQL, "Quest Objective %u has non existant Creature Id %u! Skipping.", l_ObjectiveID, l_ObjectiveObjectID);
                    continue;
                }

                if (l_ObjectiveAmount <= 0)
                {
                    TC_LOG_ERROR(LOG_FILTER_SQL, "Quest Objective %u has valid Creature Id %u but amount %u is invalid! Skipping.", l_ObjectiveID, l_ObjectiveObjectID, l_ObjectiveAmount);
                    m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
                    continue;
                }
            ...
            case QUEST_OBJECTIVE_TYPE_ITEM:
            {
                ...
                {
                    TC_LOG_ERROR(LOG_FILTER_SQL, "Quest Objective %u has non existant Item Id %u! Skipping.", l_ObjectiveID, l_ObjectiveObjectID);
                    m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
                    continue;
                }
                ...
                if (l_ObjectiveAmount <= 0)
                {
                    TC_LOG_ERROR(LOG_FILTER_SQL, "Quest Objective %u has valid Item Id %u but amount %u is invalid! Skipping.", l_ObjectiveID, l_ObjectiveObjectID, l_ObjectiveAmount);
                    m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
                    continue;
                }
            ...
            case QUEST_OBJECTIVE_TYPE_GO:
            {
                ...
                {
                    TC_LOG_ERROR(LOG_FILTER_SQL, "Quest Objective %u has non existant GameObject Id %u! Skipping.", l_ObjectiveID, l_ObjectiveObjectID);
                    m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
                    continue;
                }
                ...
                if (l_ObjectiveAmount <= 0)
                {
                    TC_LOG_ERROR(LOG_FILTER_SQL, "Quest Objective %u has valid GameObject Id %u but amount %u is invalid! Skipping.", l_ObjectiveID, l_ObjectiveObjectID, l_ObjectiveAmount);
                    m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
                    continue;
                }
            ...
            case QUEST_OBJECTIVE_TYPE_CURRENCY:
            {
                ...
                {
                    TC_LOG_ERROR(LOG_FILTER_SQL, "Quest Objective %u has non existant Currency Id %u! Skipping.", l_ObjectiveID, l_ObjectiveObjectID);
                    m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
                    continue;
                }
            ...
            case QUEST_OBJECTIVE_TYPE_SPELL:
            {
                ...
                if (!l_Spell)
                {
                    TC_LOG_ERROR(LOG_FILTER_SQL, "Quest Objective %u has non existant Spell Id %u! Skipping.", l_ObjectiveID, l_ObjectiveObjectID);
                    continue;
                }

                else if (!SpellMgr::IsSpellValid(l_Spell))
                {
                    TC_LOG_ERROR(LOG_FILTER_SQL, "Quest Objective %u has invalid Spell Id %u! Skipping.", l_ObjectiveID, l_ObjectiveObjectID);
                    m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
                    continue;
                }
                ...
                if (l_ObjectiveAmount <= 0)
                {
                    TC_LOG_ERROR(LOG_FILTER_SQL, "Quest Objective %u has valid Spell Id %u but amount %u is invalid! Skipping.", l_ObjectiveID, l_ObjectiveObjectID, l_ObjectiveAmount);
                    m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
                    continue;
                }
            ...
            case QUEST_OBJECTIVE_TYPE_FACTION_REP2:
            {
                ...
                {
                    TC_LOG_ERROR(LOG_FILTER_SQL, "Quest Objective %u has non existant Faction Id %u! Skipping.", l_ObjectiveID, l_ObjectiveObjectID);
                    m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
                    continue;
                }
            ...
            case QUEST_OBJECTIVE_TYPE_MONEY:
            {
                ...
                {
                    TC_LOG_ERROR(LOG_FILTER_SQL, "Quest Objective %u is invalid, Quest %u already has the max amount of Quest Objective type %u! Skipping.", l_ObjectiveID, l_ObjectiveQuestId, l_ObjectiveType);
                    m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
                    continue;
                }
            ...
            case QUEST_OBJECTIVE_TYPE_PET_BATTLE_ELITE:
            {
                ...
                {
                    TC_LOG_ERROR(LOG_FILTER_SQL, "Quest Objective %u has non existant Battle Pet Species %u! Skipping.", l_ObjectiveID, l_ObjectiveObjectID);
                    m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
                    continue;
                }
                ...
                ///{
                ///    TC_LOG_ERROR(LOG_FILTER_SQL, "Quest Objective %u has invalid Battle Pet Species %u, it doesn't have BATTLE_PET_FLAG_ELITE flag! Skipping.", id, objectId);
                ///    m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
                ///    continue;
                ///}
            ...
            case QUEST_OBJECTIVE_TYPE_PET_BATTLE_PVP:
            {
                ...
                if (l_ObjectiveAmount <= 0)
                {
                    TC_LOG_ERROR(LOG_FILTER_SQL, "Quest Objective %u has invalid Pet Battle PvP win amount %u! Skipping.", l_ObjectiveID, l_ObjectiveObjectID, l_ObjectiveAmount); ///< Data argument not used by format string
                    m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
                    continue;
                }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u Quest Objectives in %u ms.", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadQuestObjectiveLocales()
{
    ...
    if (!l_Result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 Quest Objective locale descriptions. DB table `locales_quest_objective` is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "Quest Objective locale Id %u has invalid Quest Objective %u! Skipping.", l_Locale, l_ObjectiveID);
            continue;
        }

        if (l_Locale >= TOTAL_LOCALES)
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "Locale Id %u for Quest Objective %u is invalid! Skipping.", l_Locale, l_ObjectiveID);
            continue;
        }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u Quest Objective visual effects in %u ms.", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadItemBonusGroup()
{
    ...
    if (!l_Result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 Item Bonus Group. DB table `item_bonus_group` is empty.");
        return;
    }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u item bonus group in %u ms.", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadItemBonusGroupLinked()
{
    ...
    if (!l_Result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 Item Bonus Group Linked. DB table `item_bonus_group_linked` is empty.");
        return;
    }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u item bonus group linked in %u ms.", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadSpellInvalid()
{
    ...
    if (!l_Result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 Spell Invalid. DB table `spell_invalid` is empty.");
        return;
    }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u Spell Invalid in %u ms.", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadSpellStolen()
{
    ...
    if (!l_Result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 Spell Stolen. DB table `spell_stolen` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (!spellInfo)
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "SpellStolen: spell (spell_id:%d) does not exist in `Spell.dbc`.", l_SpellID);
            continue;
        }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u Spell Stolen in %u ms.", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadDisabledEncounters()
{
    ...
    if (!l_Result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 disabled ranking. DB table `instance_disabled_rankings` is empty.");
        return;
    }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u disabled ranking in %u ms.", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadConversationTemplates()
{
    ...
    if (!l_Result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 conversation template. DB table `conversation_template` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (!l_LinesResult)
        {
            TC_LOG_ERROR(LogFilterType::LOG_FILTER_SERVER_LOADING, "Conversation %u doesn't have any record in conversation_lines!", l_Conversation.Entry);
            continue;
        }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u conversation templates in %u ms.", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
