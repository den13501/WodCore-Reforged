...
uint32 LootStore::LoadLootTable()
{
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "Table '%s' entry %d item %d: maxcount value (%u) to large. must be less %u - skipped", GetName(), entry, item, maxcount, std::numeric_limits<int32>::max());
    ...
    if (mincountOrRef > 0)                                  // item (quest or non-quest) entry, maybe grouped
    {
        ...
        if (int size = itemBonuses.size())
        {
            for (int i = 0; i < size; i++)
            {
                ...
                {
                    TC_LOG_ERROR(LOG_FILTER_SQL, "Table '%s' entry %d item %d: non existing item bonus %d - skipped", store.GetName(), entry, itemid, itemBonuses[i]);
                    return false;
                }
            }
        }
