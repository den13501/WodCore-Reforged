...
void WaypointMovementGenerator<Creature>::LoadPath(Creature* creature)
{
    ...
    if (!i_path)
    {
        ...
        TC_LOG_ERROR(LOG_FILTER_SQL, "WaypointMovementGenerator::LoadPath: creature %s (%lu DB GUID: " UI64FMTD ") doesn't have waypoint path id: %u", creature->GetName(), creature->GetGUID(), creature->GetDBTableGUIDLow(), path_id);
        return;
    }
...
void WaypointMovementGenerator<Creature>::OnArrived(Creature* creature)
{
    ...
    {
        //TC_LOG_DEBUG(LOG_FILTER_MAPSCRIPTS, "Creature movement start script %u at point %u for %llu.", i_path->at(i_currentNode)->event_id, i_currentNode, creature->GetGUID());
...
void FlightPathMovementGenerator::PreloadEndGrid()
{
    ...
    if (endMap)
    {
        TC_LOG_DEBUG(LOG_FILTER_GENERAL, "Preloading rid (%f, %f) for map %u at node index %u/%u", _endGridX, _endGridY, _endMapId, _preloadTargetNode, (uint32)(i_path.size() - 1));
        endMap->LoadGrid(_endGridX, _endGridY);
    }
    else
        TC_LOG_DEBUG(LOG_FILTER_GENERAL, "Unable to determine map to preload flightmaster grid");
}
