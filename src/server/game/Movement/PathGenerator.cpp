    ...
    _navMeshQuery(NULL)
{
    ...
    //TC_LOG_DEBUG(LOG_FILTER_MAPS, "++ PathGenerator::PathGenerator for %llu", _sourceUnit->GetGUID());
...
bool PathGenerator::CalculatePath(float destX, float destY, float destZ, bool forceDest, bool straightLine)
{
    ...
    //TC_LOG_DEBUG(LOG_FILTER_MAPS, "++ PathGenerator::CalculatePath() for %llu", _sourceUnit->GetGUID());
...
void PathGenerator::BuildPolyPath(G3D::Vector3 const& startPos, G3D::Vector3 const& endPos)
{
    ...
    else if (startPolyFound && !endPolyFound)
    {
        ...
        if (!suffixPolyLength || dtStatusFailed(dtResult))
        {
            ...
            TC_LOG_ERROR(LOG_FILTER_MAPS, "%lu's Path Build failed: 0 length path", _sourceUnit->GetGUID());
        }
...
void PathGenerator::ReducePathLenghtByDist(float dist)
{
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_MAPS, "PathGenerator::ReducePathLenghtByDist called before path was built");
        return;
    }
