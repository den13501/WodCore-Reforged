...
void SmartScript::ProcessAction(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellInfo* spell, GameObject* gob)
{
    ...
    switch (e.GetActionType())
    {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsCreature(*itr))
                {
                    ...
                    e.GetTargetType() == SMART_TARGET_CLOSEST_ENEMY || e.GetTargetType() == SMART_TARGET_CLOSEST_FRIENDLY)
                    {
                        ...
                        TC_LOG_DEBUG(LOG_FILTER_DATABASE_AI, "SmartScript: Action target for SMART_ACTION_SET_HOME_POS is invalid, skipping");
                }
            }
        ...
        case SMART_ACTION_GAME_EVENT_STOP:
        {
            ...
            {
                TC_LOG_DEBUG(LOG_FILTER_SQL, "SmartScript::ProcessAction: At case SMART_ACTION_GAME_EVENT_STOP, inactive event (id: %u)", eventId);
                return;
            }
        ...
        case SMART_ACTION_GAME_EVENT_START:
        {
            ...
            if (sGameEventMgr->IsActiveEvent(eventId))
            {
                TC_LOG_DEBUG(LOG_FILTER_SQL, "SmartScript::ProcessAction: At case SMART_ACTION_GAME_EVENT_START, already activated event (id: %u)", eventId);
                return;
            }
            ...
            if (ObjectList* targets = GetTargets(e, unit))
            {
                for (WorldObject* const obj : *targets)
                {
                    if (IsUnit(obj))
                    {
                        ...
                        TC_LOG_DEBUG(LOG_FILTER_SQL, "SmartScript::ProcessAction:: SMART_ACTION_RANDOM_SOUND: target: %s (%u), sound: %u, onlyself: %s",
        ...
        default:
            TC_LOG_DEBUG(LOG_FILTER_SQL, "SmartScript::ProcessAction: Entry %d SourceType %u, Event %u, Unhandled Action type %u", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
            break;
    }
...
void SmartScript::FillScript(SmartAIEventList e, WorldObject* obj, AreaTriggerEntry const* at)
{
    ...
    if (mEvents.empty() && obj)
        TC_LOG_DEBUG(LOG_FILTER_SQL, "SmartScript: Entry %u has events but no events added to list because of instance flags.", obj->GetEntry());
    if (mEvents.empty() && at)
        TC_LOG_DEBUG(LOG_FILTER_SQL, "SmartScript: AreaTrigger %u has events but no events added to list because of instance flags. NOTE: triggers can not handle any instance flags.", at->ID);
}
