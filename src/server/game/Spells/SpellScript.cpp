...
int32 SpellScript::GetAbsorbedDamage()
{
    if (!IsInTargetHook())
    {
        TC_LOG_ERROR(LOG_FILTER_TSCR, "Script: `%s` Spell: `%u`: function SpellScript::GetAbsorbedDamage was called, but function has no effect in current hook!", m_scriptName->c_str(), m_scriptSpellId);
        return 0;
    }
...
bool AuraScript::_Validate(SpellInfo const* entry)
{
    ...
    for (std::list<EffectPeriodicHandler>::iterator itr = AfterEffectPeriodic.begin(); itr != AfterEffectPeriodic.end(); ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            TC_LOG_ERROR(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match dbc effect data - handler bound to hook `AfterEffectPeriodic` of AuraScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());
