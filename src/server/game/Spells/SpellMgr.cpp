...
void SpellMgr::InitializeItemUpgradeDatas()
{
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 71 item upgrade datas in %u ms", GetMSTimeDiffToNow(oldMSTime));
}
...
void SpellMgr::LoadSpellPlaceHolder()
{
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u talent place holder in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void SpellMgr::LoadSpellUpgradeItemStage()
{
    ...
    if (!l_Result)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 spell upgrade item stage. DB table `spell_upgrade_item_stage` is empty.");
        return;
    }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u spell upgrade item stage.", l_Count);
}
