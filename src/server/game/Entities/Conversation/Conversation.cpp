...
bool Conversation::StartConversation(Unit* p_Source, uint32 p_ConversationEntry)
{
    ...
    if (!IsPositionValid())
    {
        TC_LOG_ERROR(LogFilterType::LOG_FILTER_GENERAL, "Conversation::StartConversation -> Invalid coordinates for Conversation [%u] (X: %.3f, Y: %.3f, Z: %.3f)",
            p_ConversationEntry,
            m_positionX,
            m_positionY,
            m_positionZ);
    ...
    {
        TC_LOG_ERROR(LogFilterType::LOG_FILTER_GENERAL, "Conversation::StartConversation -> Conversation [%u] isn't in an instanced map.", p_ConversationEntry);
        return false;
    }
    ...
    {
        TC_LOG_ERROR(LogFilterType::LOG_FILTER_GENERAL, "Conversation::StartConversation -> Conversation [%u] doesn't have a template.", p_ConversationEntry);
        return false;
    }
