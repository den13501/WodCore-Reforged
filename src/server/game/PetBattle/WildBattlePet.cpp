...
void WildBattlePetZonePools::LoadPoolTemplate(Field* l_Fields)
{
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_SERVER_LOADING, "No npc entry for species %u", l_PoolTemplate.Species);
        return;
    }
...
void WildBattlePetMgr::Load()
{
    ...
    do
    {
        ...
        if (l_MapID == -1)
        {
            TC_LOG_ERROR(LOG_FILTER_SERVER_LOADING, "WildBattlePetMgr::Load() no map id found for zone %u", l_ZoneID);
            continue;
        }
        ...
        for (size_t l_I = 0; l_I < m_PoolsByMap[l_MapID][l_ZoneID].m_Templates.size(); l_I++)
        {
            ...
            {
                l_Error = true;
                TC_LOG_ERROR(LOG_FILTER_SERVER_LOADING, "WildBattlePetMgr::Load() zone %u already contains a replacement for creature entry %u", l_ZoneID, l_Fields[2].GetUInt32());
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u species definitions.", l_Count);
}
