    ...
    switch (cond->SourceType)
    {
        ...
        case CONDITION_SOURCE_TYPE_SPELL_IMPLICIT_TARGET:
        {
            ...
            for (uint8 i = 0; i < SpellEffIndex::MAX_EFFECTS; ++i)
            {
                ...
                TC_LOG_ERROR(LOG_FILTER_SQL, "SourceEntry %u SourceGroup %u in `condition` table - spell %u does not have implicit targets of types: _AREA_, _CONE_, _NEARBY_, _CYLINDER_ for effect %u, SourceGroup needs correction, ignoring.", cond->SourceEntry, origGroup, cond->SourceEntry, uint32(i));
...
bool ConditionMgr::isConditionTypeValid(Condition* cond) const
{
    ...
    switch (cond->ConditionType)
    {
        ...
        case CONDITION_ACHIEVEMENT:
        {
            ...
            if (!achievement)
            {
                TC_LOG_ERROR(LOG_FILTER_SQL, "Achievement condition has non existing achievement id (%u), skipped", cond->ConditionValue1);
                return false;
            }

            if (cond->ConditionValue2)
                TC_LOG_ERROR(LOG_FILTER_SQL, "Achievement condition has useless data in value2 (%u)!", cond->ConditionValue2);
            if (cond->ConditionValue3)
                TC_LOG_ERROR(LOG_FILTER_SQL, "Achievement condition has useless data in value3 (%u)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_CLASS:
        {
        ...
        {
            if (cond->ConditionValue3)
                TC_LOG_ERROR(LOG_FILTER_SQL, "HasBuilding condition has useless data in value3 (%u)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_HAS_GARRISON_LEVEL:
        {
            if (cond->ConditionValue2)
                TC_LOG_ERROR(LOG_FILTER_SQL, "HasGarrisonLevel condition has useless data in value3 (%u)!", cond->ConditionValue2);

            if (cond->ConditionValue3)
                TC_LOG_ERROR(LOG_FILTER_SQL, "HasGarrisonLevel condition has useless data in value3 (%u)!", cond->ConditionValue3);
