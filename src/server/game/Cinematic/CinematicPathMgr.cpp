...
size_t CinematicSequenceMgr::Load()
{
    ...
    for (uint32 l_SequenceID = 0; l_SequenceID < sCinematicSequencesStore.GetNumRows(); ++l_SequenceID)
    {
        ...
        if (!l_ModelFile)
        {
            TC_LOG_ERROR(LOG_FILTER_SERVER_LOADING, "Cinematic camera '%s': does not exist!", l_ModelFileName.c_str());
            continue;
        }
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_SERVER_LOADING, "Cinematic camera '%s': invalid magic header!", l_ModelFileName.c_str());
            continue;
        }
        ...
        if (!l_Header->CameraCount || !l_Header->CameraOffset)
        {
            ...
            TC_LOG_ERROR(LOG_FILTER_SERVER_LOADING, "Cinematic camera '%s': no cameras!", l_ModelFileName.c_str());
            continue;
        }
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_SERVER_LOADING, "Cinematic camera '%s': no translations!", l_ModelFileName.c_str());
            free(l_FileBuffer);
            continue;
        }
        ...
        TC_LOG_ERROR(LOG_FILTER_SERVER_LOADING, "Cinematic camera '%s': loaded! (duration : %u ms)", l_ModelFileName.substr(l_ModelFileName.find_last_of("/") + 1).c_str(), l_Sequence->Duration);
    }
