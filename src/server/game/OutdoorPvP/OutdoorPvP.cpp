...
OutdoorGraveyard* OutdoorPvP::GetGraveyardById(uint32 p_ID)
{
    if (p_ID < m_GraveyardList.size())
    {
        if (m_GraveyardList[p_ID])
            ...
            TC_LOG_ERROR(LOG_FILTER_BATTLEFIELD, "OutdoorPvP::GetGraveyardById Id:%u not existed", p_ID);
    }
    else
        TC_LOG_ERROR(LOG_FILTER_BATTLEFIELD, "OutdoorPvP::GetGraveyardById Id:%u cant be found", p_ID);
...
void OutdoorGraveyard::SetSpirit(Creature* p_Spirit, TeamId p_Team)
{
    if (p_Spirit == nullptr)
    {
        TC_LOG_ERROR(LOG_FILTER_BATTLEFIELD, "OutdoorGraveyard::SetSpirit -> Invalid Spirit.");
        return;
    }
