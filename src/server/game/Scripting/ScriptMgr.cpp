...
void DoScriptText(int32 p_ItemTextEntry, WorldObject* p_Source, Unit* p_Target)
{
    if (!p_Source)
    {
        TC_LOG_ERROR(LOG_FILTER_TSCR, "DoScriptText entry %i, invalid Source pointer.", p_ItemTextEntry);
        return;
    }

    if (p_ItemTextEntry >= 0)
    {
        TC_LOG_ERROR(LOG_FILTER_TSCR, "DoScriptText with source entry %u (TypeId=%u, guid=%u) attempts to process text entry %i, but text entry must be negative.", p_Source->GetEntry(), p_Source->GetTypeId(), p_Source->GetGUIDLow(), p_ItemTextEntry);
        return;
    }
    ...
    if (!l_TextDatas)
    {
        TC_LOG_ERROR(LOG_FILTER_TSCR, "DoScriptText with source entry %u (TypeId=%u, guid=%u) could not find text entry %i.", p_Source->GetEntry(), p_Source->GetTypeId(), p_Source->GetGUIDLow(), p_ItemTextEntry);
        return;
    }

    TC_LOG_DEBUG(LOG_FILTER_TSCR, "DoScriptText: text entry=%i, Sound=%u, Type=%u, Language=%u, Emote=%u", p_ItemTextEntry, l_TextDatas->uiSoundId, l_TextDatas->uiType, l_TextDatas->uiLanguage, l_TextDatas->uiEmote);
    ...
    if (l_TextDatas->uiSoundId)
    {
        if (sSoundEntriesStore.LookupEntry(l_TextDatas->uiSoundId))
            ...
            TC_LOG_ERROR(LOG_FILTER_TSCR, "DoScriptText entry %i tried to process invalid sound id %u.", p_ItemTextEntry, l_TextDatas->uiSoundId);
    }
    ...
    if (l_TextDatas->uiEmote)
    {
        if (p_Source->GetTypeId() == TYPEID_UNIT || p_Source->IsPlayer())
            ...
            TC_LOG_ERROR(LOG_FILTER_TSCR, "DoScriptText entry %i tried to process emote for invalid TypeId (%u).", p_ItemTextEntry, p_Source->GetTypeId());
    }
    ...
    switch (l_TextDatas->uiType)
    {
        ...
        case CHAT_TYPE_WHISPER:
        {
            if (p_Target && p_Target->IsPlayer())
                ...
                TC_LOG_ERROR(LOG_FILTER_TSCR, "DoScriptText entry %i cannot whisper without target unit (TYPEID_PLAYER).", p_ItemTextEntry);
...
void ScriptMgr::Initialize()
{
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u C++ scripts in %u ms", GetScriptCount(), GetMSTimeDiffToNow(l_OldMSTime));
}
    ...
    : ScriptObjectImpl(p_Name), MapScript<Map>(p_MapID)
{
    if (GetEntry() && !GetEntry()->IsWorldMap())
        TC_LOG_ERROR(LOG_FILTER_TSCR, "WorldMapScript for map %u is invalid.", p_MapID);
    ...
    : ScriptObjectImpl(p_Name), MapScript<InstanceMap>(p_MapID)
{
    if (GetEntry() && !GetEntry()->IsDungeon())
        TC_LOG_ERROR(LOG_FILTER_TSCR, "InstanceMapScript for map %u is invalid.", p_MapID);
    ...
    : ScriptObjectImpl(p_Name), MapScript<BattlegroundMap>(p_MapID)
{
    if (GetEntry() && !GetEntry()->IsBattleground())
        TC_LOG_ERROR(LOG_FILTER_TSCR, "BattlegroundMapScript for map %u is invalid.", p_MapID);
