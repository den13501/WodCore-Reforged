    ...
    while (!m_scriptSchedule.empty() && (iter->first <= sWorld->GetGameTime()))
    {
        ...
        switch (step.script->command)
        {
            case SCRIPT_COMMAND_TALK:
                ...
                {
                    TC_LOG_ERROR(LOG_FILTER_TSCR, "%s invalid chat type (%u), script type %u specified, skipping.", step.script->GetDebugInfo().c_str(), step.script->Talk.ChatType, step.script->type);
                    break;
                }
