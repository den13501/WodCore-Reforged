    ...
    if (!sScriptMgr->OnItemOpen(m_Player, l_Item))
    {
        if (l_LockID)
        {
            ...
            if (!l_LockInfo)
            {
                ...
                TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "WORLD::OpenItem: item [guid = %u] has an unknown lockId: %u!", l_Item->GetGUIDLow(), l_LockID);
        ...
        if (l_Item->HasFlag(ITEM_FIELD_DYNAMIC_FLAGS, ITEM_FIELD_FLAG_WRAPPED))
        {
            ...
            {
                TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "Wrapped item %u don't have record in character_gifts table and will deleted", l_Item->GetGUIDLow());
...
void WorldSession::HandleCastSpellOpcode(WorldPacket& p_RecvPacket)
{
    ...
    if (!spellInfo)
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "WORLD: unknown spell id %u", l_SpellID);
        p_RecvPacket.rfinish(); // prevent spam at ignore packet
        return;
    }
...
void WorldSession::HandlePetCancelAuraOpcode(WorldPacket& p_RecvPacket)
{
    ...
    if (!l_SpellInfo)
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "WORLD: unknown PET spell id %u", l_SpellID);
        return;
    }
    ...
    if (!l_Pet)
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "HandlePetCancelAura: Attempt to cancel an aura for non-existant pet %u by player '%s'", uint32(GUID_LOPART(l_PetGUID)), GetPlayer()->GetName());
        return;
    }

    if (l_Pet != GetPlayer()->GetGuardianPet() && l_Pet != GetPlayer()->GetCharm())
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "HandlePetCancelAura: Pet %u is not a pet of player '%s'", uint32(GUID_LOPART(l_PetGUID)), GetPlayer()->GetName());
        return;
    }
...
void WorldSession::HandleUseToyOpcode(WorldPacket& p_RecvData)
{
    ...
    if (!l_ItemProto)
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "WORLD: unknown item id %u", l_ItemID);
        p_RecvData.rfinish();
        return;
    }
