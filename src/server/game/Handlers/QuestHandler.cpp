...
void WorldSession::HandleQuestgiverStatusQueryOpcode(WorldPacket & p_RecvData)
{
    ...
    if (!l_QuestGiver)
    {
        TC_LOG_INFO(LOG_FILTER_NETWORKIO, "Error in CMSG_QUESTGIVER_STATUS_QUERY, called for not found questgiver (Typeid: %u GUID: %u)", GuidHigh2TypeId(GUID_HIPART(l_QuestGiverGUID)), GUID_LOPART(l_QuestGiverGUID));
        return;
    }
    ...
    switch (l_QuestGiver->GetTypeId())
    {
        ...
        default:
            TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "QuestGiver called for unexpected type %u", l_QuestGiver->GetTypeId());
            break;
    }
...
void WorldSession::HandleQuestgiverChooseRewardOpcode(WorldPacket& p_RecvData)
{
    ...
    if (l_Quest->GetQuestPackageID() == 0)
    {
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "Error in CMSG_QUESTGIVER_CHOOSE_REWARD: player %s (guid %d) tried to get invalid reward (%u) (probably packet hacking)", m_Player->GetName(), m_Player->GetGUIDLow(), l_RewardEntry);
            return;
        }
    }
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "HACK ALERT: Player %s (guid: %u) is trying to complete quest (id: %u) but he has no right to do it!",
