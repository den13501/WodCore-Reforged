...
void WorldSession::HandleEjectPassenger(WorldPacket& p_RecvData)
{
    ...
    if (IS_PLAYER_GUID(l_Guid))
    {
        ...
        if (!l_Player)
        {
            TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "Player %u tried to eject player %u from vehicle, but the latter was not found in world!", GetPlayer()->GetGUIDLow(), GUID_LOPART(l_Guid));
            return;
        }

        if (!l_Player->IsOnVehicle(vehicle->GetBase()))
        {
            TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "Player %u tried to eject player %u, but they are not in the same vehicle", GetPlayer()->GetGUIDLow(), GUID_LOPART(l_Guid));
            return;
        }
        ...
        if (l_VehicleSeat->IsEjectable())
            ...
            TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "Player %u attempted to eject player %u from non-ejectable seat.", GetPlayer()->GetGUIDLow(), GUID_LOPART(l_Guid));
    }
    ...
    else if (IS_CREATURE_GUID(l_Guid))
    {
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "Player %u tried to eject creature guid %u from vehicle, but the latter was not found in world!", GetPlayer()->GetGUIDLow(), GUID_LOPART(l_Guid));
            return;
        }

        if (!l_Unit->IsOnVehicle(vehicle->GetBase()))
        {
            TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "Player %u tried to eject unit %u, but they are not in the same vehicle", GetPlayer()->GetGUIDLow(), GUID_LOPART(l_Guid));
            return;
        }
        ...
        if (l_VehicleSeat->IsEjectable())
        {
            ...
            TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "Player %u attempted to eject creature GUID %u from non-ejectable seat.", GetPlayer()->GetGUIDLow(), GUID_LOPART(l_Guid));
    }
}
    ...
    if (Vehicle* l_Vehicle = GetPlayer()->GetVehicle())
    {
        if (VehicleSeatEntry const* l_VehicleSeat = l_Vehicle->GetSeatForPassenger(GetPlayer()))
        {
            if (l_VehicleSeat->CanEnterOrExit())
                ...
                TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "Player %u tried to exit vehicle, but seatflags %u (ID: %u) don't permit that.", GetPlayer()->GetGUIDLow(), l_VehicleSeat->m_ID, l_VehicleSeat->m_flags);
        }
    }
}
