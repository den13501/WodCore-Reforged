...
bool WorldSession::processChatmessageFurtherAfterSecurityChecks(std::string& msg, uint32 lang)
{
    if (lang != LANG_ADDON)
    {
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "Player %s (GUID: %u) sent a chatmessage with an invalid link: %s", GetPlayer()->GetName(),
...
void WorldSession::HandleMessagechatOpcode(WorldPacket& p_RecvData)
{
    ...
    switch (p_RecvData.GetOpcode())
    {
        ...
        case CMSG_CHAT_MESSAGE_RAID_WARNING:
            ...
            break;
        default:
            TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "HandleMessagechatOpcode : Unknown chat opcode (%u)", p_RecvData.GetOpcode());
            p_RecvData.hexlike();
            return;
    }
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "CHAT: Wrong message type received: %u", l_Type);
        p_RecvData.rfinish();
        return;
    }
    ...
    switch (l_Type)
    {
        ...
        default:
            TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "CHAT: unknown message type %u, lang: %u", l_Type, l_Language);
            break;
    }
}
...
void WorldSession::HandleAddonMessagechatOpcode(WorldPacket& p_RecvData)
{
    ...
    switch (p_RecvData.GetOpcode())
    {
        ...
        case CMSG_CHAT_ADDON_MESSAGE_WHISPER:
            ...
            break;
        default:
            TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "HandleAddonMessagechatOpcode: Unknown addon chat opcode (%u)", p_RecvData.GetOpcode());
            p_RecvData.hexlike();
            return;
    }
    ...
    switch (l_Type)
    {
        ...
        default:
        {
            TC_LOG_ERROR(LOG_FILTER_GENERAL, "HandleAddonMessagechatOpcode: unknown addon message type %u", l_Type);
            break;
        }
    }
}
...
{
    TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "Opcode %u", p_Packet.GetOpcode());
}
