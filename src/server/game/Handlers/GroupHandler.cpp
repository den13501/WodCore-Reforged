...
void WorldSession::HandleGroupInviteResponseOpcode(WorldPacket& p_RecvData)
{
    ...
    if (l_Accept)
    {
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "HandleGroupAcceptOpcode: player %s(%d) tried to accept an invite to his own group", GetPlayer()->GetName(), GetPlayer()->GetGUIDLow());
            return;
        }
...
void WorldSession::HandlePartyUninviteOpcode(WorldPacket& p_RecvData)
{
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "WorldSession::HandlePartyUninviteOpcode: leader %s(%d) tried to uninvite himself from the group.", GetPlayer()->GetName(), GetPlayer()->GetGUIDLow());
        return;
    }
...
void WorldSession::HandleRequestPartyMemberStatsOpcode(WorldPacket& recvData)
{
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "Player %u (%s) sent CMSG_REQUEST_PARTY_MEMBER_STATS for player %u (%s) whos is not in the same group!",
