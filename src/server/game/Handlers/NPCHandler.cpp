...
void WorldSession::SendTrainerList(uint64 p_NpcGUID, const std::string& p_Title)
{
    ...
    if (!l_Unit)
    {
        TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "WORLD: SendTrainerList - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(p_NpcGUID)));
        return;
    }
    ...
    {
        TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "WORLD: SendTrainerList - (GUID: %u) NO CREATUREINFO!", GUID_LOPART(p_NpcGUID));
        return;
    }
...
void WorldSession::HandleTrainerBuySpellOpcode(WorldPacket& p_Packet)
{
    ...
    if (!l_Unit)
    {
        TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "WORLD: HandleTrainerBuySpellOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(l_TrainerGUID)));
        return;
    }
...
void WorldSession::HandleSpiritHealerActivateOpcode(WorldPacket& p_Packet)
{
    ...
    if (!l_Unit)
    {
        TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "WORLD: HandleSpiritHealerActivateOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(l_Healer)));
        return;
    }
...
void WorldSession::HandleBinderActivateOpcode(WorldPacket& p_RecvData)
{
    ...
    if (!l_Unit)
    {
        TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "WORLD: HandleBinderActivateOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(l_NpcGUID)));
        return;
    }
