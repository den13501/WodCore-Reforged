...
void WorldSession::HandleUpgradeGarrisonOpcode(WorldPacket& p_RecvData)
{
    ...
    if (!l_Unit)
    {
        TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "WORLD: HandleUpgradeGarrisonOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(l_NpcGUID)));
        return;
    }

    if (!l_Garrison->CanUpgrade())
    {
        TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "WORLD: HandleUpgradeGarrisonOpcode - Can't upgrade");
        return;
    }
...
void WorldSession::HandleGarrisonMissionNPCHelloOpcode(WorldPacket& p_RecvData)
{
    ...
    if (!l_Unit)
    {
        TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "WORLD: HandleGarrisonMissionNPCHelloOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(l_NpcGUID)));
        return;
    }
...
void WorldSession::HandleGarrisonRequestSetMissionNPC(WorldPacket& p_RecvData)
{
    ...
    if (!l_Unit)
    {
        TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "WORLD: HandleGarrisonRequestSetMissionNPC - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(l_NpcGUID)));
        return;
    }
...
void WorldSession::HandleGarrisonPurchaseBuildingOpcode(WorldPacket& p_RecvData)
{
    ...
    if (!l_Unit)
    {
        TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "WORLD: HandleGarrisonPurchaseBuildingOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(l_NpcGUID)));
        return;
    }
...
void WorldSession::HandleGarrisonCancelConstructionOpcode(WorldPacket& p_RecvData)
{
    ...
    if (!l_Unit)
    {
        TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "WORLD: HandleGarrisonCancelConstructionOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(l_NpcGUID)));
        return;
    }
...
void WorldSession::HandleGarrisonStartMissionOpcode(WorldPacket& p_RecvData)
{
    ...
    if (!l_Unit)
    {
        TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "WORLD: HandleGarrisonStartMissionOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(l_NpcGUID)));
        return;
    }
...
void WorldSession::HandleGarrisonCompleteMissionOpcode(WorldPacket& p_RecvData)
{
    ...
    if (!l_Unit)
    {
        TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "WORLD: HandleGarrisonCompleteMissionOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(l_NpcGUID)));
        return;
    }
...
void WorldSession::HandleGarrisonMissionBonusRollOpcode(WorldPacket& p_RecvData)
{
    ...
    if (!l_Unit)
    {
        TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "WORLD: HandleGarrisonMissionBonusRollOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(l_NpcGUID)));
        return;
    }
...
void WorldSession::HandleGarrisonGenerateRecruitsOpcode(WorldPacket& p_RecvData)
{
    ...
    if (l_Unit == nullptr)
    {
        TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "WORLD: HandleGarrisonMissionBonusRollOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(l_GUID)));
        return;
    }
...
void WorldSession::HandleGarrisonAssignFollowerToBuilding(WorldPacket& p_RecvData)
{
    ...
    if (!l_Creature)
    {
        TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "WORLD: HandleGarrisonAssignFollowerToBuilding - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(l_NpcGUID)));
        return;
    }
...
void WorldSession::HandleGarrisonRemoveFollowerFromBuilding(WorldPacket& p_RecvData)
{
    ...
    if (!l_Creature)
    {
        TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "WORLD: HandleGarrisonRemoveFollowerFromBuilding - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(l_NpcGUID)));
        return;
    }
