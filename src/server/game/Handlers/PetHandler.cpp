...
void WorldSession::HandlePetAction(WorldPacket & p_RecvPacket)
{
    ...
    if (l_Pet == nullptr)
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "HandlePetAction: Pet (GUID: %u) doesn't exist for player '%s'", uint32(GUID_LOPART(l_PetGUID)), GetPlayer()->GetName());
        return;
    }

    if (l_Pet != GetPlayer()->GetFirstControlled())
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "HandlePetAction: Pet (GUID: %u) does not belong to player '%s'", uint32(GUID_LOPART(l_PetGUID)), GetPlayer()->GetName());
        return;
    }
...
void WorldSession::HandlePetStopAttack(WorldPacket &p_RecvPacket)
{
    ...
    if (!l_Pet)
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "HandlePetStopAttack: Pet %u does not exist", uint32(GUID_LOPART(l_PetGUID)));
        return;
    }

    if (l_Pet != GetPlayer()->GetPet() && l_Pet != GetPlayer()->GetCharm())
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "HandlePetStopAttack: Pet GUID %u isn't a pet or charmed creature of player %s", uint32(GUID_LOPART(l_PetGUID)), GetPlayer()->GetName());
        return;
    }
...
void WorldSession::HandlePetSetAction(WorldPacket & p_RecvPacket)
{
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "HandlePetSetAction: Unknown pet (GUID: %u) or pet owner (GUID: %u)", GUID_LOPART(l_PetGUID), m_Player->GetGUIDLow());
        return;
    }
    ...
    if (!l_CharmInfo)
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "WorldSession::HandlePetSetAction: object (GUID: %u TypeId: %u) is considered pet-like but doesn't have a charminfo!", l_Pet->GetGUIDLow(), l_Pet->GetTypeId());
        return;
    }
...
void WorldSession::HandlePetCastSpellOpcode(WorldPacket& p_RecvPacket)
{
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "HandlePetCastSpellOpcode: Pet %u isn't pet of player %s .", uint32(GUID_LOPART(l_PetGUID)), GetPlayer()->GetName());
        return;
    }
    ...
    if (!l_SpellInfo)
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "WORLD: unknown PET spell id %i", l_SpellID);
        return;
    }
