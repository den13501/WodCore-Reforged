...
void WorldSession::HandlePetitionBuyOpcode(WorldPacket& p_Packet)
{
    ...
    if (!l_Creature)
    {
        TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "WORLD: HandlePetitionBuyOpcode - Unit (GUID: %u) not found or you can't interact with him.", GUID_LOPART(l_UnitGUID));
        return;
    }
...
void WorldSession::HandlePetitionShowSignOpcode(WorldPacket& p_Packet)
{
    ...
    if (!l_Result)
    {
        TC_LOG_DEBUG(LOG_FILTER_PLAYER_ITEMS, "Petition %u is not found for player %u %s", GUID_LOPART(l_ItemGUID), GetPlayer()->GetGUIDLow(), GetPlayer()->GetName());
        return;
    }
...
void WorldSession::SendPetitionQueryOpcode(uint64 l_PetitionGUID)
{
    ...
    {
        TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "CMSG_PETITION_QUERY failed for petition (GUID: %u)", GUID_LOPART(l_PetitionGUID));
        return;
    }
...
void WorldSession::HandlePetitionSignOpcode(WorldPacket& p_Packet)
{
    ...
    if (!l_Result)
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "Petition %u is not found for player %u %s", GUID_LOPART(l_PetitionGUID), GetPlayer()->GetGUIDLow(), GetPlayer()->GetName());
        return;
    }
...
void WorldSession::HandleTurnInPetitionOpcode(WorldPacket& p_Packet)
{
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "Player %s (guid: %u) tried to turn in petition (guid: %u) that is not present in the database", m_Player->GetName(), m_Player->GetGUIDLow(), GUID_LOPART(l_PetitionGuid));
        return;
    }
...
void WorldSession::SendPetitionShowList(uint64 p_GUID)
{
    ...
    if (!l_Creature)
    {
        TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "WORLD: HandlePetitionShowListOpcode - Unit (GUID: %u) not found or you can't interact with him.", uint32(GUID_LOPART(p_GUID)));
        return;
    }
