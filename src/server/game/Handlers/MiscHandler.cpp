...
void WorldSession::HandleRepopRequestOpcode(WorldPacket& p_RecvData)
{
    ...
    {
        TC_LOG_DEBUG(LogFilterType::LOG_FILTER_NETWORKIO, "HandleRepopRequestOpcode: got request after player %s(%d) was killed and before he was updated", m_Player->GetName(), m_Player->GetGUIDLow());
        m_Player->KillPlayer();
    }
...
void WorldSession::HandleSetActionBarToggles(WorldPacket& p_Packet)
{
    ...
    {
        if (l_ActionBar != 0)
            TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "WorldSession::HandleSetActionBarToggles in not logged state with value: %u, ignored", uint32(l_ActionBar));
        return;
    }
...
void WorldSession::HandleWorldTeleportOpcode(WorldPacket& recvData)
{
    ...
    {
        TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "Player '%s' (GUID: %u) in flight, ignore worldport command.", GetPlayer()->GetName(), GetPlayer()->GetGUIDLow());
        return;
    }
...
void WorldSession::HandleFarSightOpcode(WorldPacket& p_Packet)
{
    ...
    else
    {
        if (WorldObject * l_Target = m_Player->GetViewpoint())
            ...
            TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "Player %s requests non-existing seer " UI64FMTD, m_Player->GetName(), m_Player->GetGuidValue(PLAYER_FIELD_FARSIGHT_OBJECT));
    }
...
void WorldSession::HandleTimeSyncResp(WorldPacket& p_RecvData)
{
    ...
    if (l_Counter != m_Player->m_timeSyncCounter - 1)
        TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "Wrong time sync counter from player %s (cheater?)", m_Player->GetName());
...
void WorldSession::HandleSetDungeonDifficultyOpcode(WorldPacket & recvData)
{
    ...
    if (!difficultyEntry)
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "WorldSession::HandleSetDungeonDifficultyOpcode: %d sent an invalid instance mode %d!", m_Player->GetGUIDLow(), mode);
        return;
    }

    if (difficultyEntry->InstanceType != MAP_INSTANCE)
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "WorldSession::HandleSetDungeonDifficultyOpcode: %d sent an non-dungeon instance mode %d!", m_Player->GetGUIDLow(), difficultyEntry->ID);
        return;
    }

    if (!(difficultyEntry->Flags & DIFFICULTY_FLAG_CAN_SELECT))
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "WorldSession::HandleSetDungeonDifficultyOpcode: %d sent unselectable instance mode %d!", m_Player->GetGUIDLow(), difficultyEntry->ID);
        return;
    }
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "WorldSession::HandleSetDungeonDifficultyOpcode: player (Name: %s, GUID: %u) tried to reset the instance while player is inside!", m_Player->GetName(), m_Player->GetGUIDLow());
        return;
    }
    ...
    if (group)
    {
        if (group->IsLeader(m_Player->GetGUID()))
        {
            for (GroupReference* itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
            {
                ...
                {
                    TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "WorldSession::HandleSetDungeonDifficultyOpcode: player %d tried to reset the instance while group member (Name: %s, GUID: %u) is inside!", m_Player->GetGUIDLow(), groupGuy->GetName(), groupGuy->GetGUIDLow());
                    return;
                }
            }
...
void WorldSession::HandleSetRaidDifficultyOpcode(WorldPacket& p_RecvData)
{
    ...
    if (!difficultyEntry)
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "WorldSession::HandleSetDungeonDifficultyOpcode: %d sent an invalid instance mode %u!",
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "WorldSession::HandleSetDungeonDifficultyOpcode: %d sent an non-dungeon instance mode %u!",
    ...
    if (!(difficultyEntry->Flags & DIFFICULTY_FLAG_CAN_SELECT))
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "WorldSession::HandleSetDungeonDifficultyOpcode: %d sent unselectable instance mode %u!",
                       m_Player->GetGUIDLow(), difficultyEntry->ID);
        return;
    }
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "WorldSession::HandleSetDungeonDifficultyOpcode: %d sent not matching legacy difficulty %u!",
                       m_Player->GetGUIDLow(), difficultyEntry->ID);
        return;
    }
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "WorldSession::HandleSetRaidDifficultyOpcode: player %d tried to reset the instance while inside!", m_Player->GetGUIDLow());
        return;
    }
...
void WorldSession::HandleInstanceLockResponse(WorldPacket& recvPacket)
{
    ...
    {
        TC_LOG_INFO(LOG_FILTER_NETWORKIO, "InstanceLockResponse: Player %s (guid %u) tried to bind himself/teleport to graveyard without a pending bind!", m_Player->GetName(), m_Player->GetGUIDLow());
        return;
    }
...
void WorldSession::HandleObjectUpdateFailedOpcode(WorldPacket & p_Packet)
{
    ...
    TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "Object update failed for object " UI64FMTD " (%s) for player %s (%u)", uint64(l_ObjectGUID), l_WorldObject ? l_WorldObject->GetName() : "object-not-found", GetPlayerName().c_str(), GetGuidLow());
}
