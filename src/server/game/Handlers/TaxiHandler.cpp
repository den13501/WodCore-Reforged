...
void WorldSession::SendTaxiStatus(uint64 p_Guid)
{
    ...
    if (!l_Unit)
    {
        TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "WorldSession::SendTaxiStatus - Unit (GUID: %u) not found.", uint32(GUID_LOPART(p_Guid)));
        return;
    }
...
void WorldSession::HandleTaxiQueryAvailableNodes(WorldPacket& p_RecvPacket)
{
    ...
    if (!unit)
    {
        TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "WORLD: HandleTaxiQueryAvailableNodes - Unit (GUID: %u) not found or you can't interact with him.", uint32(GUID_LOPART(l_UnitGuid)));
        return;
    }
    // remove fake death
...
void WorldSession::HandleActivateTaxiOpcode(WorldPacket& p_RecvPacket)
{
    ...
    if (!l_Npc)
    {
        TC_LOG_DEBUG(LOG_FILTER_NETWORKIO, "WORLD: HandleActivateTaxiOpcode - Unit (GUID: %u) not found or you can't interact with it.", uint32(GUID_LOPART(l_VendorGuid)));
        return;
    }
